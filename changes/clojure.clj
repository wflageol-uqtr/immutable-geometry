...
(defn point [x y & {:keys [color] :or {color :black}}]
      {:shape ::point :x x :y y :color color})

(defn size [w h]
      {:shape ::size :w w :h h})

(defn rectangle [x y w h & {:keys [color] :or {color :black}}]
      {:shape ::rectangle :x x :y y :w w :h h :color color})
      
(defmulti print-geo (fn [geo-object] (geo-object :shape)))
(defmethod print-geo ::point [point]
  (println (format "Point(%d, %d, %s)" (point :x) (point :y) (point :color))))
(defmethod print-geo ::size [size]
  (println (format "Size(%d, %d)" (size :w) (size :h))))
(defmethod print-geo ::rectangle [rect]
  (println (format "Rectangle(%d, %d, %d, %d, %s)" (rect :x) (rect :y) (rect :w) (rect :h) (rect :color))))
...
(let [p (point 2 2 :color :yellow)
      ...
      r (rectangle 2 2 3 4 :color :green)
      r2 (move r 1 2)
      r3 (rotate r)
