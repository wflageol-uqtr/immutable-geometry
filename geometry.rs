...
impl fmt::Display for Rectangle {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "#<Rectangle ({}, {}, {}, {})>", self.x, self.y, self.w, self.h)
    }
}
...
impl Movable<Rectangle> for Rectangle {
    ...
    fn rotate_around(&self, origin: &Point) -> Rectangle {
        ...
        let top_left = Point { x: self.x, y: self.y };
        let top_right = Point { x: self.x + self.w, y: self.y };
        let bot_left = Point { x: self.x, y: self.y + self.h };
...
impl Rectangle {
    ...
    fn intersect(&self, target: &Rectangle) -> Option<Rectangle> {
        if self.does_intersect(target) {
            ...
            Some (Rectangle { x: left, y: top, w: right - left, h: bot - top })
        } else {
            None
        }
    }
...
fn main() {
    ...
    let p3 = p.rotate();
    let p4 = p.rotate_around(&Point { x: 0, y: 0});

    let s = Size { w: 4, h: 6 };
    let s2 = s.scale(3.0);
    let s3 = s.fit_into(&Size { w: 3, h: 3});

    let r = Rectangle { x: 2, y: 2, w: 3, h: 4};
    let r2 = r.translate(1, 2);
    let r3 = r.rotate();
    let r4 = r.rotate_around(&Point { x: 0, y: 0});

    let r5 = r.scale(5.0);
    let r6 = r.fit_into(&Size { w: 8, h: 8});

    let r7 = r.intersect(&Rectangle { x: 4, y: 3, w: 3, h: 2 });
